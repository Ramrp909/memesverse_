...
export default function SettingsDrawer({
  open, onClose, dark, setDark, isLoggedIn, onSignIn, onSignOut,
}: {
  open: boolean;
  onClose: () => void;
  dark: boolean;
  ...
  return (
    <>
      ...
      >
        {/* drawer header */}
        <div className="flex items-center justify-between px-5 py-4 border-b flex-shrink-0" style={{ borderColor: c.borderSub }}>
          <Logo />
          <button onClick={onClose}
            className="w-8 h-8 flex items-center justify-center rounded-full transition-all hover:opacity-70"
            style={{ background: c.btnBg, color: c.textMuted }}>
            <X size={15} />
          </button>
        </div>
        ...
        {isLoggedIn && (
          <div className="px-5 py-5 border-b flex items-center gap-4 flex-shrink-0" style={{ borderColor: c.borderSub }}>
            ...
            <div className="min-w-0">
              <p className="text-base font-bold truncate" style={{ fontFamily: D2, color: c.text }}>MemeUser</p>
              <p className="text-xs mt-0.5" style={{ fontFamily: B2, color: c.textMuted }}>@memeuser · Member</p>
              <div className="flex items-center gap-2 mt-1.5">
                ...
                <span className="text-[10px]" style={{ fontFamily: B2, color: c.textDim }}>84.2K followers</span>
              </div>
            </div>
          </div>
        )}
        ...
        <div className="flex-1 overflow-y-auto px-5 py-5 space-y-6" style={{ scrollbarWidth: "none" }}>
          ...
          <div>
            <p className="text-[10px] font-bold uppercase tracking-widest mb-3"
              style={{ fontFamily: D2, color: c.textDim }}>Appearance</p>
            <div className="grid grid-cols-2 gap-2">
              {([["Dark", true], ["Light", false]] as [string, boolean][]).map(([label, val]) => (
                <button key={label} onClick={() => setDark(val)}
                  className="py-2.5 rounded-xl text-sm font-semibold border transition-all"
                  style={{ fontFamily: B2,
                    background: dark === val ? "#6366f1" : c.btnBg,
                    color: dark === val ? "white" : c.btnText,
                    borderColor: dark === val ? "#6366f1" : c.border }}>
          ...
          <div>
            <p className="text-[10px] font-bold uppercase tracking-widest mb-3"
              style={{ fontFamily: D2, color: c.textDim }}>Account</p>
            {isLoggedIn ? (
